...
double GetAsyE(double sig0, double sigpi, double sig0E, double sigpiE){

double s0E = sig0E/100.*sig0;
double piE = sigpiE/100.*sigpi;

double numE = sqrt(pow(s0E,2)+pow(piE,2));
double denE = sqrt(pow(s0E,2)+pow(piE,2));
double asy =100.*(-sigpi+sig0)/(sigpi+sig0);
double asyE = asy*sqrt(pow(numE/(-sigpi+sig0),2)+pow(denE/(sigpi+sig0),2));


cout <<" uncert asy:  "<<asyE<<endl;
return fabs(asyE);

}


void sensitivity_a_b_v2_25(){
/*
 * Kin I
...
//Q2=0.05 alternative setting with E =1.1 to avoid elastic peak
   double W=1.230,la=0.7,lb=0.7, kin=2.2,kout ,kth,q2=0.25,thgg=0.*d2r,phcm0=0.*d2r,phcmpi=180.*d2r;
    double la1=0.1,la2=1.3,lb1=0.1,lb2=1.3;
    int model =5,ffact=4;
    double gp, gplab,Q20;
    Calckinvars(W,q2,kin,kth,kout,gp,gplab,Q20);
   ...
   // cout<<"result is : " <<alpha_driver_dr_(&la,&qq2[0]) <<alpha_driver_dr_(&la,&qq2[1])<<endl;
    double S1=0.711,M1=1.,E1=1.;
       ...
       if(xs0!=xs0||xspi!=xspi) continue;
        ...
        TGraphErrors * gpasy  =new TGraphErrors();
        TGraphErrors * gp0  =new TGraphErrors();
        TGraphErrors * gppi =new TGraphErrors();


        double sig0EE =0.03, sigpiEE=0.03;


        gp0->SetPoint(0,130,g0->Eval(130));
        gp0->SetPoint(1,150,g0->Eval(150));
        gp0->SetPoint(2,170,g0->Eval(170));

        gp0->SetPointError(0,0,g0->Eval(130)*sig0EE);
        gp0->SetPointError(1,0,g0->Eval(150)*sig0EE);
        gp0->SetPointError(2,0,g0->Eval(170)*sig0EE);

        
        gppi->SetPoint(0,115,g180->Eval(115));
        gppi->SetPoint(1,130,g180->Eval(130));
        gppi->SetPoint(2,150,g180->Eval(150));
        gppi->SetPoint(3,170,g180->Eval(170));

        gppi->SetPointError(0,0,g180->Eval(115)*sigpiEE);
        gppi->SetPointError(1,0,g180->Eval(130)*sigpiEE);
        gppi->SetPointError(2,0,g180->Eval(150)*sigpiEE);
        gppi->SetPointError(3,0,g180->Eval(170)*sigpiEE);


        gpasy->SetPoint(0,125,gasy->Eval(130));
        gpasy->SetPoint(1,145,gasy->Eval(150));
        gpasy->SetPoint(2,165,gasy->Eval(170));


        gpasy->SetPointError(0,0.,GetAsyE(g0->Eval(130),g180->Eval(130),3,3));
        gpasy->SetPointError(1,0.,GetAsyE(g0->Eval(150),g180->Eval(150),3,3));
        gpasy->SetPointError(2,0.,GetAsyE(g0->Eval(170),g180->Eval(170),3,3));
       ...
       gppi->Draw("psame");
        ...
        gasy->GetYaxis()->SetTitle("Asymmetry (%)");
